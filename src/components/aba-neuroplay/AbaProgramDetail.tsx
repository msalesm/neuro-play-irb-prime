...
const METHOD_SHORT: Record<string, string> = {
  ...
  prompting: 'Prompting', shaping: 'Shaping',
};

const METHOD_LABELS: Record<string, string> = {
  dtt: 'Discrete Trial Training (DTT)', net: 'Natural Environment Teaching (NET)',
  task_analysis: 'Análise de Tarefas', prompting: 'Prompting', shaping: 'Shaping',
};

const STATUS_LABELS: Record<string, string> = {
  in_progress: 'Em Progresso', mastered: 'Dominada', paused: 'Pausada', discontinued: 'Descontinuada',
};
...
export function AbaProgramDetail({ programId, childId, onBack }: Props) {
  ...
  const [form, setForm] = useState({
    skill_id: '', teaching_method: 'dtt', target_level: '80', success_criteria: '',
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Habilidades, sessões, metas e observações</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="skills" className="space-y-4">
        <TabsList className="flex-wrap h-auto gap-1">
          <TabsTrigger value="skills" className="flex items-center gap-1.5">
            <Activity className="h-4 w-4" /> Habilidades
          </TabsTrigger>
        ...
        <TabsContent value="sessions">
          <AbaSessionManager programId={programId} childId={childId} />
        </TabsContent>

        {/* Goals Tab */}
        <TabsContent value="goals">
          <AbaGoalsPanel programId={programId} />
        </TabsContent>
...
function InterventionRow({ intervention, onClick }: { intervention: any; onClick: () => void }) {
  ...
  const trendColor = stats.trend === 'up' ? 'text-emerald-600' : stats.trend === 'down' ? 'text-destructive' : 'text-muted-foreground';
