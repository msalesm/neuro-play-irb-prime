...
export function ModernGameCard({
  ...
  const content = (
    <Card className={`relative overflow-hidden transition-smooth hover:scale-105 hover:shadow-glow ${!unlocked ? 'opacity-70' : ''}`}>
      ...
      {/* Decorative elements */}
      <div className="absolute bottom-2 left-2 w-8 h-8 bg-primary-foreground/10 rounded-full blur-md" />
      ...
      {progress && (
        <div className="absolute top-3 right-3 z-10">
          <Badge className="bg-background/30 text-primary-foreground border-primary-foreground/20 backdrop-blur-sm">
            {progress.current}/{progress.total}
          </Badge>
        </div>
      )}
      
      <CardContent className="relative p-4 h-32 flex flex-col justify-between text-primary-foreground">
        ...
        {(difficulty || duration) && (
          <div className="flex gap-1.5">
            {difficulty && (
              <Badge variant="secondary" className="bg-primary-foreground/20 text-primary-foreground border-primary-foreground/20 text-xs py-0.5 px-2">
                {difficulty}
              </Badge>
            )}
            {duration && (
