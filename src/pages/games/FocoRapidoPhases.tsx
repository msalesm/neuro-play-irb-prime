...
import { focoRapidoPhases } from '@/data/game-phases/foco-rapido-phases';
import { useGamePhaseProgress } from '@/hooks/useGamePhaseProgress';
import type { GamePhase } from '@/types/game-phase';

export default function FocoRapidoPhases() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const childProfileId = searchParams.get('childProfileId');
  
  const { progress, isLoading, unlockPhase } = useGamePhaseProgress('foco-rapido', childProfileId || undefined);
  const [phases, setPhases] = useState<GamePhase[]>(focoRapidoPhases);

  useEffect(() => {
    if (progress) {
      const updatedPhases = unlockPhase(focoRapidoPhases).map(phase => ({
        ...phase,
  ...
  const handlePhaseSelect = (phase: GamePhase) => {
    ...
    navigate(`/games/foco-rapido-play?${params.toString()}`);
  };
    ...
    <div className="container mx-auto p-4 md:p-6 max-w-6xl">
      <div className="mb-6">
        ...
        <div className="text-center mb-6">
          <h1 className="text-4xl font-bold mb-2">⚡ Foco Rápido</h1>
          <p className="text-muted-foreground mb-4">
            Melhore velocidade de processamento e atenção seletiva
          </p>
      ...
      <Card className="mt-6 p-6">
        ...
        <div className="space-y-2 text-sm text-muted-foreground">
          ...
          <p>⭐⭐ 2 Estrelas: Tempo médio abaixo do limite</p>
          <p>⭐⭐⭐ 3 Estrelas: Precisão acima de 90%</p>
        </div>
