...
export default function ExecutiveProcessingPhases() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const childProfileId = searchParams.get('childProfileId');
  
  const { progress, isLoading, unlockPhase } = useGamePhaseProgress('executive-processing', childProfileId || undefined);
  const [phases, setPhases] = useState<GamePhase[]>(executiveProcessingPhases);

  useEffect(() => {
    if (progress) {
      const updatedPhases = unlockPhase(executiveProcessingPhases).map(phase => ({
        ...phase,
  ...
  const handlePhaseSelect = (phase: GamePhase) => {
    ...
    navigate(`/games/executive-processing-play?${params.toString()}`);
  };
    ...
    <div className="container mx-auto p-4 md:p-6 max-w-6xl">
      <div className="mb-6">
        ...
        <div className="text-center mb-6">
          <h1 className="text-4xl font-bold mb-2">🎯 Desvio de Impulsos</h1>
          <p className="text-muted-foreground mb-4">
            Pratique controle inibitório respondendo seletivamente
          </p>
      ...
      <Card className="mt-6 p-6">
        ...
        <div className="space-y-2 text-sm text-muted-foreground">
          ...
          <p>⭐⭐ 2 Estrelas: Atingir 85% de precisão</p>
          <p>⭐⭐⭐ 3 Estrelas: Atingir 92% de precisão</p>
        </div>
